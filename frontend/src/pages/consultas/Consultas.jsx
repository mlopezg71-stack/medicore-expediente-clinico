...
const Consultas = () => {
    ...
    const confirmarCambioEstado = (consulta) => {
        setConsultaSeleccionada(consulta);
        setMostrarModal(true);
    };

    const cambiarEstadoConsulta = async () => {
        try {
            const nuevoEstado =
                consultaSeleccionada.estado === "ACTIVA"
                    ? "INACTIVA"
                    : "ACTIVA";

            await axiosClient.put(
                `/consultas/${consultaSeleccionada.id_consulta}`,
                {
                    estado: nuevoEstado
                }
            );
            ...
            setMensaje(
                nuevoEstado === "ACTIVA"
                    ? "Consulta reactivada correctamente"
                    : "Consulta anulada correctamente"
            );
            ...
            setMensaje("Error al cambiar estado de la consulta");
        }
    };

    const consultasFiltradas = consultas.filter((consulta) => {
        const paciente = `${consulta.expediente?.paciente?.nombres || ""} ${consulta.expediente?.paciente?.apellidos || ""}`;
        const medico = consulta.usuarioMedico?.nombre_completo || "";
        const expediente = consulta.expediente?.numero_expediente || "";
    ...
    const modulos = esPaciente
        ? [
            ...
            },
            {
                ...
                icono: <FaStethoscope />,
                ruta: "/paciente/consultas",
                visible: true
            },
            {
                nombre: "Mis recetas",
                icono: <FaPrescriptionBottleAlt />,
                ruta: "/paciente/recetas",
                visible: true
            },
            {
                ...
                <div style={styles.modalOverlay}>
                    <div style={styles.modal}>
                        <h2 style={styles.modalTitle}>
                            {consultaSeleccionada?.estado === "ACTIVA"
                                ? "Anular consulta médica"
                                : "Reactivar consulta médica"}
                        </h2>

                        <p style={styles.modalText}>
                            {consultaSeleccionada?.estado === "ACTIVA"
                                ? "¿Desea anular esta consulta médica? La consulta seguirá registrada en el expediente clínico."
                                : "¿Desea reactivar esta consulta médica?"}
                        </p>
            ...
            <main style={styles.main}>
                <nav style={styles.navbar}>
                    <h1 style={styles.title}>
                        {esPaciente ? "Mis Consultas Médicas" : "Consultas Médicas"}
                    </h1>
                </nav>
                ...
                <section style={styles.topActions}>
                        ...
                        <input
                            ...
                            placeholder={
                                esPaciente
                                    ...
                                    : "Buscar consulta, paciente, médico o expediente..."
                            }
                            value={busqueda}
                    ...
                    {!esPaciente && (
                        <button
                            ...
                            onClick={() => navigate("/consultas/crear")}
                        >
                            <FaPlus />
                            Nueva Consulta
                        </button>
                            ...
                            {consultasFiltradas.length === 0 ? (
                                <tr>
                                    <td colSpan="7" style={styles.emptyRow}>
                                        No se encontraron consultas médicas.
                                    </td>
                                </tr>
                            ) : (
                                    ...
                                    <tr key={consulta.id_consulta}>
                                        <td style={styles.td}>
                                            {consulta.expediente?.paciente
                                                ? `${consulta.expediente.paciente.nombres} ${consulta.expediente.paciente.apellidos}`
                                                : "Paciente no registrado"}
                                        </td>

                                        <td style={styles.td}>
                                            {consulta.expediente?.numero_expediente || "N/A"}
                                        </td>

                                        <td style={styles.td}>
                                            {consulta.usuarioMedico?.nombre_completo || "Sin médico"}
                                        </td>
                                        ...
                                        <td style={styles.td}>
                                            {consulta.tipo_consulta || "GENERAL"}
                                        </td>
                                            ...
                                            <div style={styles.acciones}>
                                                ...
                                                {!esPaciente && (
                                                    <button
                                                        ...
                                                        onClick={() =>
                                                            navigate(`/consultas/editar/${consulta.id_consulta}`)
                                                        }
                                                    >
                                                        <FaEdit />
                                                    </button>
                                                ...
                                                <button
                                                    ...
                                                    onClick={() =>
                                                        navigate(
                                                            esPaciente
                                                                ? `/paciente/consultas/ver/${consulta.id_consulta}`
                                                                : `/consultas/ver/${consulta.id_consulta}`
                                                        )
                                                    }
                                                >
                                                    <FaEye />
                                                ...
                                                {!esPaciente && consulta.estado === "ACTIVA" && (
                                                    <button
                                                        style={{
                                                            ...styles.actionButton,
                                                            backgroundColor: "#eb5757"
                                                        }}
                                                        onClick={() => confirmarCambioEstado(consulta)}
                                                    >
                                                        <FaTrash />
                                                    </button>
                                                )}

                                                {!esPaciente && consulta.estado === "INACTIVA" && (
                                                    <button
                                                        style={{
                                                            ...styles.actionButton,
                                                            backgroundColor: "#f4bd4f",
